...
const Onboarding = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (status === "never submit") {
          ...
          setReadOnly(true);
          fetchProfile(token, userId); // Pass userId to fetchProfile
  ...
  const fetchProfile = async (token, userId) => {
    try {
      ...
      // Fetch employee profile and documents
      ...
      if (res.data.documents && Array.isArray(res.data.documents)) {
        ...
        console.log("Document map created:", docMap);
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    const token = localStorage.getItem("token");
    try {
      // Get the decoded token to ensure we have the userId
      const decoded = jwtDecode(token);
      ...
      // Check if user is a permanent resident or has visa info
      if (formData.isPermanentResident === "yes") {
        if (!formData.greenCardStatus) {
          window.alert("Please select your residency status");
          return;
        }
      } else if (formData.isPermanentResident === "no") {
        if (!formData.visaType) {
          window.alert("Please select your visa type");
          return;
        }
        // Check for other visa fields
        if (formData.visaType === "Other" && !formData.otherVisaTitle) {
          window.alert("Please specify your visa title");
          return;
        }
        if (!formData.visaStartDate || !formData.visaEndDate) {
          window.alert("Please provide visa start and end dates");
          return;
        }
      } else {
        window.alert(
          "Please indicate whether you are a permanent resident or citizen"
        );
        return;
      }
      ...
      // Build the payload
      const payload = {
        userId: currentUserId, // Use the freshly decoded ID
        email,
        firstName: formData.firstName,
        lastName: formData.lastName,
        middleName: formData.middleName || "",
        preferredName: formData.preferedName || "", // Note: There may be a typo here (prefered vs preferred)
        ssn: formData.ssn,
        dateOfBirth: formData.dob,
        gender: formData.gender,
        currentAddress: {
          building: "", // Add this if your backend requires it
        ...
        contactInfo: {
          ...
          workPhone: "", // Add this if your backend requires it
        },
        isPermanentResident: formData.isPermanentResident === "yes",
        residencyType:
          formData.isPermanentResident === "yes"
            ? formData.greenCardStatus
            : null,
        visaType:
          formData.isPermanentResident === "no" ? formData.visaType : null,
        visaTitle:
          formData.isPermanentResident === "no" && formData.visaType === "Other"
            ? formData.otherVisaTitle
            : null,
        startDate:
          formData.isPermanentResident === "no" ? formData.visaStartDate : null,
        endDate:
          formData.isPermanentResident === "no" ? formData.visaEndDate : null,
        reference: {
      ...
      if (err.response && err.response.data) {
        ...
        window.alert(
          `Submission failed: ${
            err.response.data.message || "Please check your form data"
          }`
        );
      } else {
        ...
        <Form>
          ...
          {formData.visaType === "F1(CPT/OPT)" && (
            <DocumentUpload
              ...
              mode={status}
              documentId={documentMap["OPT Receipt"]?._id}
            />
          )}
          ...
          <DocumentUpload
            ...
            mode={status}
            documentId={documentMap["Profile Picture"]?._id}
          />
          ...
          <DocumentUpload
            ...
            mode={status}
            documentId={documentMap["Driver's License"]?._id}
          />
          ...
          <DocumentUpload
            ...
            mode={status}
            documentId={documentMap["Work Authorization"]?._id}
          />
